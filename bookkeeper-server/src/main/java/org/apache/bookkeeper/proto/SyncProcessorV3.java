...
class SyncProcessorV3 extends PacketProcessorBaseV3 implements Runnable {
    ...
    private SyncResponse getSyncResponse() {
        ...
        BookkeeperInternalCallbacks.SyncCallback wcb = new BookkeeperInternalCallbacks.SyncCallback() {
            @Override
            public void syncComplete(int rc, long ledgerId, long lastAddSynced, BookieSocketAddress addr, Object ctx) {
                logger.info("sync ledg "+ledgerId+", lastAddSyncedEntry:"+lastAddSynced);
                ...
                syncResponse.setLastAddSynced(lastAddSynced);
        ...
        StatusCode status;
        try {
            requestProcessor.bookie.sync(ledgerId, wcb, channel, masterKey);
            status = StatusCode.EOK;
        } catch (IOException e) {
            logger.error("Error syncing ledger {}", new Object[] { ledgerId }, e);
            status = StatusCode.EIO;
        } catch (BookieException.LedgerFencedException e) {
            logger.error("ledger fenced during synch of ledger {}", new Object[] { ledgerId }, e);
            status = StatusCode.EFENCED;
        } catch (BookieException e) {
            logger.error("unauthorized access during synch of ledger {}", new Object[] { ledgerId }, e);
            status = StatusCode.EUA;
        } catch (Throwable t) {
            logger.error("Unexpected exception while syncing ledger {} ", new Object[] { ledgerId}, t);
